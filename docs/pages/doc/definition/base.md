...
* Для добавления используются директивы [tmp_dir](#tmp_dir) и [build_dir](#build_dir), которые различаются местом размещения внешней директории. 

### Scratch образ
Пустой docker образ, используемый в качестве окружения приложения по умолчанию, если пользователь не указал его ([docker.from](#dockerfrom-image-cache_version-cache_version)).
